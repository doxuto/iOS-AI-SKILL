...
1. For features added after Swift 6.2, check `references/swift-6.3-features.md`.
...
- Swift Testing does *not* support UI tests – XCTest must be used there. It also has no equivalent of `XCTMetric`/`measure(metrics:)`, so performance tests stay in XCTest too. Both frameworks coexist in one project without conflict.
...
- `references/swift-6.3-features.md` - issue severity, test cancellation, and image attachments.
