...
1. If the project targets a recent OS, check for newer API and correct availability gating using `references/platform-releases.md`.

For the exact HIG numbers behind any of these checks — Dynamic Type point sizes, minimum tap targets, layout margins, contrast ratios, semantic colors — use the `apple-hig-specs` skill rather than recalling a figure.
...
- iOS 26 and iOS 27 both exist. iOS 26 is a sensible deployment target for a new app that still needs one release of back-compatibility; iOS 27 is the default when the project can require the newest OS. Ask if the project has not stated one.
- Any API newer than the deployment target must be gated with `#available` and given a fallback. See `references/platform-releases.md` for what shipped in which release.
- Target Swift 6.2 or later, using modern Swift concurrency. Swift 6.3 and 6.4 are supersets; prefer their newer API when the toolchain supports it.
...
- `references/platform-releases.md` - what shipped in iOS 26 and iOS 27, and how to gate it.
